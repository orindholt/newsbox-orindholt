let cardStorage=localStorage.cards?JSON.parse(localStorage.cards):[];"on"==localStorage.darkmode&&document.querySelector("body").classList.add("dark-theme"),localStorage.cards?document.querySelector(".Nav__inbox").setAttribute("data-text",cardStorage.length):document.querySelector(".Nav__inbox").setAttribute("data-text","0");const key="CsZGAx8GwP1dlmWuflb94puYcOmdpaXP",categories=["world","health","sports","business","travel"],idArray=[];cardStorage.forEach(t=>idArray.push(t.id));const ax=t=>{t=`https://api.nytimes.com/svc/topstories/v2/${t}.json?api-key=`+key;return axios(t,{mode:"cors"}).then(t=>t.data.results)};function shortenStr(t,e){return t.length<=e?t:t.substr(0,t.lastIndexOf(" ",e))}categories.forEach(t=>{if("off"!=localStorage[""+t]){let e=document.createElement("section");e.classList.add("Section"),document.querySelector("#main").append(e),e.innerHTML+=`
        <div class="d-f gap-3 p-2 ai-center">
            ...
            <button class="ml-auto Section__btn show-hide"><i class="fas fa-chevron-down Section__chevron"></i></button>
                ...
                </p>`}})}}),document.querySelector(".Main").addEventListener("touchstart",o=>{let n=o.target.parentElement,i=o.target.closest(".Card");if(n.classList.contains("Card__text")||n.parentElement.classList.contains("Card__text")){n=o.target.closest(".Card__text");let e=Math.floor(o.touches[0].clientX),a,s=i.querySelector(".Card__btn"),r=s.offsetWidth;n.addEventListener("touchmove",t=>{a=Math.floor(t.touches[0].clientX),a<e&&a>e-r&&(n.style.transform=`translateX(${a-e}px)`)}),n.addEventListener("touchend",()=>{a<e-r/3?n.style.transform=`translateX(-${r}px)`:n.style.transform="translateX(0px)"}),s.onclick=()=>{s.disabled=!0;var t={id:i.id,section:i.getAttribute("data-section")};cardStorage.push(t),localStorage.cards=JSON.stringify(cardStorage),document.querySelector(".Nav__inbox").setAttribute("data-text",cardStorage.length),Toastify({className:"Toast_save",text:`'${shortenStr(i.querySelector(".Card__title").textContent,20)}...' was archived.`,duration:3e3,stopOnFocus:!1,gravity:"top",position:"center"}).showToast(),i.classList.add("del-animation"),i.style.pointerEvents="none";let e=i.closest(".Card__container");e.style.maxHeight=e.offsetHeight-100+"px",setTimeout(()=>{i.remove()},1200)}}else n.classList.contains("show-hide")&&(n.onclick=()=>{let t=o.target.closest(".Section"),e=t.querySelector(".Card__container");"rotate(-90deg)"!=o.target.style.transform?(o.target.style.transform="rotate(-90deg)",e.style.pointerEvents="none",e.classList.remove("show-animation"),e.classList.add("hide-animation")):(o.target.style.transform="rotate(0deg)",e.style.pointerEvents="auto",e.classList.remove("hide-animation"),e.classList.add("show-animation"))})});