let cardStorage=localStorage.cards?JSON.parse(localStorage.cards):[];const key="CsZGAx8GwP1dlmWuflb94puYcOmdpaXP",sections=[];cardStorage.forEach(t=>sections.push(t.section));const categories=shortenArr(sections),idArray=[];cardStorage.forEach(t=>idArray.push(t.id));const ax=t=>{t=`https://api.nytimes.com/svc/topstories/v2/${t}.json?api-key=`+key;return axios(t,{mode:"cors"}).then(t=>t.data.results)};function shortenStr(t,e){return t.length<=e?t:t.substr(0,t.lastIndexOf(" ",e))}function shortenArr(t){const e=[];return t.forEach(t=>{e.includes(t)||e.push(t)}),e}categories.length||(document.querySelector("#main").innerHTML='<p class="ta-center">Sorry, nothing here..</p>'),categories.forEach(t=>{let e=document.createElement("section");e.classList.add("Section"),document.querySelector("#main").append(e),e.innerHTML+=`
    ...
    </div>`;let a=0,s=document.createElement("div");s.classList.add("Card__container"),e.append(s),ax(t).then(t=>{t.forEach(t=>{"admin"!=t.section&&idArray.includes(t.short_url)&&(s.innerHTML+=`
